...
根據持有貨幣的量和時間所得出的幣齡來決定下一個區塊由誰挖出，而每次由你挖出區塊後，你的幣齡就會歸零重新計算，而被清空之幣齡達到特定數量後也可獲得獎勵，可以預防51%攻擊，不必擔心未來如果區塊鏈算力降低時，整個區塊鏈網路的51%以上算力都被控制在特定人手下。

PoA \( Proof of Authority\)  
有別於PoW \(Proof-of-Work\)需要解數學難題來產生block，PoA是依靠預設好的認證節點\( Authority Nodes \)，負責產生區塊。  
可設定認證節點 \( Authority Node \)數量以及指定產生區塊的平均時間等等。
