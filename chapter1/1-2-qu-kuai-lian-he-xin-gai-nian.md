...
區塊鏈系統使用密碼學方法\( 後面章節會提到實作 \)設計了一道複雜的數學題，只要有正確答案，就可以成爲鏈上的最新區塊。這個答案被節點成功計算出來後，會一起被打包進區塊。答案是一個數字，得到答案的唯一方法就是不斷地進行雜湊計算。哪個節點先得到了答案，它就可以儘速廣播給其他節點新的區塊內容，並且將它的算出的區塊放到鏈上。
