...
類似剛才提到的CBC模式，並且可以將區塊密碼變為可同步進行的串流加密法，且訊息無需進行填充到一定長度。
...
稱為Counter mode，CTR模式也稱為計數器模式，每個區塊對應一個累加的計數器，通過計數器來生成加密密鑰串流，計數器可以是任意保證長時間不產生重複輸出的函數。下圖的Nonce加上Counter當做一個改變的數值，Nonce和前面幾種模式的IV類似，每次加密都需要隨機生成。而計數器Counter是累加的。CTR模式特點是每組加密都是獨立的，不依賴前一組，這就意味著在生成計數器後，每個分組可以並行計算。
