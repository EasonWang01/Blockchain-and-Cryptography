...
最簡單的加密模式即為（Electronic codebook，ECB）模式。需要加密的訊息按照設定的大小被分為數個區塊，並對每個區塊進行獨立加密。但缺點在於同樣的明文區塊會被加密成相同的密文區塊，因此並不推薦用於現代的加密協定中。
...
```
將要加密的 plain text 分為多個區塊，並用同一支金鑰進行加密的動作。
```
