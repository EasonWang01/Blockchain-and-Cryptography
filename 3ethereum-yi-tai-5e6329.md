...
以太坊的代幣稱為以太幣\(ETH\)，可用來交易與支付執行智能合約的手續費，但手續費是以Gas來計算，其計算方式為
...
相當於最多允許用多少單位個Gas來執行這個交易或是合約，當合約的邏輯越多時，要執行它會需要耗費越多區塊鏈上的資源，所以自然會消耗較高單位個Gas Limit，而Gas Limit 對於同一個Function來說是固定的，也就是不管是什麼時間來執行它，都會是固定單位的。
