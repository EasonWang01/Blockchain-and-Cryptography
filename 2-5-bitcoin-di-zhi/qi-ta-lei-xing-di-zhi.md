...
bytes = decode_xkey('xpub661MyMwAqRbcEbRitzSU39AKFRmHiQfYMarq5z78fkwMa5KP67dzcNkQq8nobSyLsQx29Y1YeLbwJfRgJcxf5C1a1uuzfKkMChVeNPJ6kWs')
...
// 取得xkey之public key 與 private key 或 單獨public key
if (ser.key_bytes[0] === 1) {
	// private key
...
let index = "00000000"; // 這邊預設都是只產生index0
let HMAC_data = public_key + index;
let HMAC_key = ser.chain_code;

HMAC_SHA512 = crypto.createHmac('sha512', Buffer.from(HMAC_key, 'hex')).update(Buffer.from(HMAC_data, 'hex')).digest('hex');
console.log(HMAC_SHA512)
// HMAC-SHA512之左側與右側
...
// 算出之child public key
let child_publickey = bytesToHex(publicKeyBytesCompressed)
console.log(child_publickey)
// 用 parent 的public key 算出 finger_print
var RIPEMD160 = require('ripemd160')
sha256 = crypto.createHash('sha256').update(Buffer.from(public_key, 'hex')).digest('hex')
...
let child_index = [0,0,0,0]
let xPub_Bytes = generate_xkey(mainnet_pub);
checksum = crypto.createHash('sha256').update(Buffer.from(xPub_Bytes)).digest()
checksum = crypto.createHash('sha256').update(checksum).digest().slice(0, 4)
checksumArray = []
checksum.forEach(buf => {
...
let xPub_array = xPub_Bytes.concat(checksumArray)
let xPub = base58.encode(xPub_array)
console.log('--xPub--')
console.log(xPub)
...
function generate_xkey(_keyType) {
	return (
		...
		_keyType === mainnet_pri ? 0 : null, // 如果為xPrv要加上0
		...hexToDecimalArray(_keyType === mainnet_pri ? child_private : child_publickey), // key
		].filter(n => n !== null) // 將 null 元素移除
	)	
} 
